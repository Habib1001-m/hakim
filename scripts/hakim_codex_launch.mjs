...
export function parseArgs(args) {
  ...
  if (options.apply && options.json) {
    throw new Error('--json cannot be combined with --apply because Codex uses interactive stdout');
  }
  for (const arg of options.passthrough) {
    if (arg === '--cd' || arg === '-C' || arg.startsWith('--cd=')) {
      throw new Error('--cd/-C is controlled by Hakim and cannot be overridden');
    }
    if (
...
export function buildCodexLaunchPlan(options, root = ROOT, environment = process.env) {
  ...
  for (const [label, targetPath, expectedType, nextSafeAction] of [
    ...
    ['MARKETPLACE', marketplacePath, 'file', 'Restore the repository-local Codex marketplace declaration before launching.'],
  ...
  try {
    ...
    return fail(base, 'MANIFEST_INVALID_JSON', `Repair the Codex plugin manifest: ${error.message}`);
  }
  try {
    marketplaceJson = JSON.parse(fs.readFileSync(marketplacePath, 'utf8'));
  } catch (error) {
    return fail(base, 'MARKETPLACE_INVALID_JSON', `Repair the Codex marketplace declaration: ${error.message}`);
  }
  ...
  if (manifestJson.name !== 'hakim') {
    return fail(withContracts, 'MANIFEST_NAME_MISMATCH', 'The Codex plugin manifest must identify the hakim plugin.');
  }
  if (manifestJson.version !== hakimVersion) {
    return fail(withContracts, 'MANIFEST_VERSION_MISMATCH', 'Synchronize the Codex plugin manifest with the canonical Hakim version.');
  }
  if (marketplaceJson.name !== 'hakim-local') {
    return fail(withContracts, 'MARKETPLACE_NAME_MISMATCH', 'The repository-local marketplace must be named hakim-local.');
  }
  if (!marketplaceEntry) {
    return fail(withContracts, 'MARKETPLACE_ENTRY_MISSING', 'Restore the hakim entry in the repository-local Codex marketplace.');
  }
  if (marketplaceEntry.source?.source !== 'local' || marketplaceEntry.source?.path !== './plugins/codex') {
    return fail(withContracts, 'MARKETPLACE_SOURCE_MISMATCH', 'Restore the local marketplace source path to ./plugins/codex.');
  }
  if (marketplaceEntry.policy?.installation !== 'AVAILABLE') {
    return fail(withContracts, 'MARKETPLACE_POLICY_MISMATCH', 'Restore the Hakim marketplace installation policy to AVAILABLE.');
  }

  const binaryResolved = resolveExecutable(options.binary, environment);
  if (!binaryResolved) {
    return fail(withContracts, 'BINARY_NOT_FOUND', `Install Codex or provide an executable with --binary. Requested: ${options.binary}`);
  }
  ...
  return {
    ...
    next_safe_action: options.apply
      ? 'Launch Codex in the validated working directory; use /plugins and /hooks to verify one hakim@hakim-local installation and one trusted SessionStart hook.'
      : 'Review the binary, working directory, marketplace contract, and argv; rerun with --apply to launch Codex.',
  };
}
...
export function launchCodex(options, root = ROOT, dependencies = {}) {
  ...
  if (child.error) {
    return {
      ...
      next_safe_action: 'Resolve the Codex launch error and rerun the dry-run before trying --apply again.',
    };
  }
  if (child.signal) {
    return {
      ...
      next_safe_action: 'Inspect why Codex was interrupted before retrying.',
    };
  }
  if (child.status !== 0) {
    return {
      ...
      child_process_started: true,
      exit_code: child.status,
      next_safe_action: 'Review the Codex output and resolve the non-zero exit before retrying.',
    };
  }
  return {
    ...plan,
    ...
    child_process_started: true,
    exit_code: 0,
    next_safe_action: 'Codex exited successfully. The launcher did not install, activate, trust, or persist the Hakim plugin.',
  };
}

export function formatText(result) {
  return [
    'Hakim Codex Launcher',
    `MODE=${result.mode}`,
...
function usage() {
  return [
    ...
    '',
    'Dry-run is the default. --apply starts Codex in the validated working directory',
    'using shell-free argv. Plugin installation, activation, and hook trust remain',
    'host-UI-managed. --cd/-C and approval/sandbox bypass flags are refused.',
  ].join('\n');
}
