...
export function buildReport(results, version, scope = 'FULL', nativeAcceptance = readNativeAcceptance()) {
  ...
  return {
    ...
    next_safe_action: failed.length > 0
      ...
      : nativeOverall !== 'PASS'
        ? 'Resolve the current native-host acceptance projection before any future evaluator relaunch decision.'
        : 'POST-BETA-R1 is complete. Do not relaunch external evaluator recruitment without a separate explicit product decision.',
    checks: results,
  };
}
