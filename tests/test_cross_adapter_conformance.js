...
const check = spawnSync('node', [path.join(root, 'scripts/check_cross_adapter_conformance.mjs')], {
  cwd: root,
  encoding: 'utf8',
});
...
try {
  ...
  assert.match(copilotPrompts, /Use Hakim capability hakim-review\./);
  assert.doesNotMatch(copilotPrompts, /\n\/hakim-review/);
} finally {
  fs.rmSync(tempRoot, { recursive: true, force: true });
}

console.log('test_cross_adapter_conformance.js: ok');
