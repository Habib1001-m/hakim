...
|---|---|---|---|
| Codex | Native Git marketplace plugin with six skills and SessionStart activation | `codex plugin marketplace add https://github.com/Habib1001-m/hakim.git --ref 5d00039479f2f11b7fe30ccf2385e70ce24553c3` | Codex `0.131.0+` is the maintained compatibility floor; frozen beta.4 live-host acceptance remains `NOT_RUN`; central OpenAI Plugin Directory listing is separate and not claimed |
| Claude Code | Native marketplace plugin with commands, hidden canonical skills, lifecycle hooks, and specialized agents | `claude plugin marketplace add https://github.com/Habib1001-m/hakim.git#5d00039479f2f11b7fe30ccf2385e70ce24553c3` | frozen beta.4 live-host acceptance remains `NOT_RUN`; installation scope, managed policy, permissions, cache, and trust remain authoritative |
| GitHub Copilot CLI | Native marketplace plugin with six skills, five custom agents, and R3.2 lifecycle presence/mode/subagent continuity | `copilot plugin marketplace add Habib1001-m/hakim#5d00039479f2f11b7fe30ccf2385e70ce24553c3` | frozen beta.4 acceptance remains separate; Copilot CLI 1.0.75 R3.2 evidence is bounded development evidence through F04, not candidate acceptance |
| OpenCode | Guarded project-local plugin with exact manifest, bounded create/adopt/transactional-upgrade/removal, ownership sentinels, quarantine verification, and no-clobber rollback | `npx --yes --package=github:Habib1001-m/hakim#5d00039479f2f11b7fe30ccf2385e70ce24553c3 hakim-opencode install` | Node `>=22` for the Git bootstrap; frozen beta.4 live-host acceptance remains `NOT_RUN`; no npm registry/global installer or `opencode.json` mutation is claimed |
...
Both currently report `HOLD_FOR_LIVE_HOST_EVIDENCE`, but they are not interchangeable. A host reaches `PASS` only after a real install/start/invocation journey is accepted for the exact version and source SHA being claimed.
