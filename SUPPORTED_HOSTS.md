...
Hakim `1.0.0-beta.4` is public beta software. Support means the repository maintains a documented, gated product surface for the host; it does not imply universal compatibility, current live-host acceptance, or stable-release authorization.
...
- The beta.4 candidate currently records all four maintained paths as `NOT_RUN`, so overall current acceptance is `HOLD_FOR_LIVE_HOST_EVIDENCE`.
