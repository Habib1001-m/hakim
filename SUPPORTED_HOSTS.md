...
- The beta.3 candidate currently records all four maintained paths as `NOT_RUN`, so overall current acceptance is `HOLD_FOR_LIVE_HOST_EVIDENCE`.
- Accepted beta.1 and frozen beta.2 evidence remains bounded to those exact historical candidates and must not be reused as beta.3 proof.
