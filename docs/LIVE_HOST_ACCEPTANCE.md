...
This document defines the public-safe workflow for accepting Hakim `1.0.0-beta.2` on a real supported coding host. The current beta.2 projection is intentionally `HOLD_FOR_LIVE_HOST_EVIDENCE` until fresh candidate-specific journeys are reviewed and accepted.
...
The beta.2 OpenCode path is currently `NOT_RUN`. Accepted beta.1 host evidence, including the previous bounded-sentinel OpenCode journey, is preserved in `conformance/history/native-host-acceptance-1.0.0-beta.1.json` and is not reused to promote beta.2.
