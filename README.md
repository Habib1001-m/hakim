...
Codex, Claude Code, GitHub Copilot CLI, and the guarded project-local OpenCode runtime each have accepted real-host evidence recorded in [`conformance/native-host-acceptance.json`](conformance/native-host-acceptance.json). That bounded evidence does not imply universal compatibility, stable-release authorization, or central marketplace publication. The new Git-backed OpenCode bootstrap added during remediation is structurally tested but must receive separate real-host evidence before that exact first-run transport is treated as independently accepted.
